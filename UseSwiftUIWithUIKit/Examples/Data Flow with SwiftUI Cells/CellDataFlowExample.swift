    ...
    var body: some View {
        HStack {
            if isEditable {
                ...
                Text(condition.text.isEmpty ? "Condition" : condition.text)
                    .foregroundStyle(condition.text.isEmpty ? Color.secondary : Color.primary)
                Spacer()
            }
        }
        .frame(minHeight: 28) // Work around for a known issue where UIHostingConfiguration minSize isn't applied correctly by default. For more information, see the iOS 16 Release Notes. (85233971)
    }
}
